...
$.koshkilJS.components.bibliotecaUpload={
    myDropzone : null,
    ...
    helpers: {
        saveRecordIfNeeded:function (mensaje,callBack) {
            ...
            if (gal_relacionado=='') {
                $.koshkilJS.helpers.askConfirmation(mensaje,function() {
                    ...
                    } else if (typeof $.koshkilJS.helpers.validarRegistro === 'function') {
                        ...
                        $.ajax({
                            ...
                            success:function(data) {
                                if (data.status=='error') {
                                    ...
                                    $('#detalles #accion').val('actualizar');
        ...
        guardarDocumento:function() {
            ...
            $.ajax({
                url:$.koshkilJS.components.bibliotecaUpload.ajaxURL,
                type:'post',
                data: postData,
                success: function(data) {
    ...
    init: function() {
        var ajaxURL=$.koshkilJS.components.bibliotecaUpload.ajaxURL;
        var dzForm=$('#dropzone-biblioteca');
        $.koshkilJS.dropZone.options.url=$(dzForm).attr('action');
        $.koshkilJS.dropZone.options.params={
            'action':$(dzForm).find('[name=dzaction]').val(),
        ...
        maxFiles=$.koshkilJS.components.bibliotecaUpload.maxFiles-$('.fake-dropzone .dz-preview.dz-image-preview').length;
        $('.fake-dropzone').data('remaining-images',maxFiles);
        $('.fake-dropzone .dz-preview.dz-image-preview').each(function() {
            $(this).find('button[data-dz-properties]').attr('type','button').click($.koshkilJS.components.bibliotecaUpload.listeners.clickDocumento);
            $(this).find('button[data-dz-remove]').attr('type','button').click($.koshkilJS.components.bibliotecaUpload.listeners.clickRemove);
        });

        $("button#procesar-dropzone").on("click", function(e) {
            e.preventDefault();
            e.stopPropagation();
            $.koshkilJS.components.bibliotecaUpload.helpers.saveRecordIfNeeded('Antes de guardar las imagenes seleccionadas, debe grabarse el registro en curso de creaci&oacute;n<br/>&iquest;Desea continuar?',function(){
                $.koshkilJS.components.bibliotecaUpload.myDropzone.processQueue();
            })
        });
    }
};
...
$.koshkilJS.dropZone={
    options: {
        ...
        parallelUploads: 4,
        maxFiles: maxFiles,
        maxFilesize: $.koshkilJS.components.bibliotecaUpload.maxFileSize,
        paramName:'documento',
        addRemoveLinks:true,
        dictRemoveFile:'Eliminar',
        dictMaxFilesExceeded:'Solo puede asignarse un m\xE1ximo de '+$.koshkilJS.components.bibliotecaUpload.maxFiles+' im\xE1genes',
        ...
        init: function() {
            ...
            this.on("successmultiple", function(files, response) {
                ...
                $('.dropzone .dz-preview.dz-image-preview.dz-success, .dropzone .dz-preview.dz-file-preview.dz-success').each(function() {
                    ...
                    $(newPreview).find('button[data-dz-properties]').attr('type','button').removeClass('hidden').click($.koshkilJS.components.bibliotecaUpload.listeners.clickDocumento);
                    $(newPreview).find('button[data-dz-remove]').attr('type','button').removeClass('hidden').click($.koshkilJS.components.bibliotecaUpload.listeners.clickRemove);
...
$(document.body).ready(function() {
    Dropzone.autoDiscover = false;
    $('#editDocumento #guardarDocumento').click($.koshkilJS.components.bibliotecaUpload.helpers.guardarDocumento);
    $.koshkilJS.components.bibliotecaUpload.myDropzone = new Dropzone('#dropzone-biblioteca', $.koshkilJS.dropZone.options);
});
